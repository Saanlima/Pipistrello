 ...
 R_RenderMaskedSegRange(drawseg_t * ds, int x1, int x2) {
	...
	if (curline->linedef->flags & ML_DONTPEGBOTTOM) {
		dc_texturemid =
		    ...
		    backsector->floorheight ? frontsector->
		    floorheight : backsector->floorheight;
		dc_texturemid = dc_texturemid + textureheight[texnum] - viewz;
	} else {
		dc_texturemid =
		    frontsector->ceilingheight <
		    backsector->ceilingheight ? frontsector->
...
void R_RenderSegLoop(void)
{
	...
	for (; rw_x < rw_stopx; rw_x++) {
		...
		if (segtextured) {
			...
			angle =
			    (rw_centerangle +
			     xtoviewangle[rw_x]) >> ANGLETOFINESHIFT;
			texturecolumn =
			    rw_offset - FixedMul(finetangent[angle],
						 rw_distance);
...
void R_StoreWallRange(int start, int stop) {
	...
	if (stop > start) {
		...
		if (rw_distance < FRACUNIT / 2) {
			...
			ds_p->scale1 =
			    FixedDiv(projection, gxt - gyt) << detailshift;
		}
#endif
	...
	if (!backsector) {
		...
		if (worldlow > worldbottom) {
			// bottom texture
			bottomtexture =
			    texturetranslation[sidedef->bottomtexture];
	...
	if (backsector) {
		...
		if (worldhigh < worldtop) {
			pixhigh =
			    (centeryfrac >> 4) - FixedMul(worldhigh, rw_scale);
			pixhighstep = -FixedMul(rw_scalestep, worldhigh);
		}

		if (worldlow > worldbottom) {
			pixlow =
			    (centeryfrac >> 4) - FixedMul(worldlow, rw_scale);
