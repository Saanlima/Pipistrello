...
static boolean ConvertMus(byte *musdata, int len, char *filename)
{
    ...
    if (result == 0)
    {
        ...
        if (M_WriteFile(filename, outbuf, outbuf_len) == 0)
            fprintf(stderr, "M_WriteFile failed!\n");
    }
    else
...
// Called from main loop to add MIDI data to music fifo
void I_PlayTrack(void)
{
    ...
    // Only playing track 0
    track = &tracks[0];
    
    // OPL2 FIFO less than half full and not paused?
    while ((SOUND0_STATUS & 0x30) == 0)
    {
...
void I_PlaySong(void *handle, int looping)
{
    ...
    if (MIDI_NumTracks(file) > 1)
    {
        fprintf(stderr, "MIDI file contains more than 1 track!\n");
        return;
    }
    ...
    if (tracks == NULL)
    {
        fprintf(stderr, "MIDI tracks malloc failed!\n");
        return;
    }
    ...
    track = &tracks[0];
    track->iter = MIDI_IterateTrack(file, 0);
    ...
    // Start filling music fifo with MIDI data
    I_PlayTrack();
}
...
{
//    fprintf(stdout, "OPL register 0x%02x set to 0x%02x\n", reg, value);
    SOUND0_OPL3 = (reg<<8 | value) & 0xffff;
}

void OPL_WriteDelay(unsigned short delay)
{
//    fprintf(stdout, "OPL delay: %d ms\n", delay);
    SOUND0_OPL3 = delay<<16;
}
