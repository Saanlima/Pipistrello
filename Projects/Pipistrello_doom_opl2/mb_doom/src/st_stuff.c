...
void ST_updateFaceWidget(void)
{
	...
	if (priority < 8) {
		if (plyr->damagecount
		    && plyr->attacker && plyr->attacker != plyr->mo) {
			// being attacked
			priority = 7;
...
void ST_doPaletteStuff(void)
{
	...
	if (palette != st_palette) {
		st_palette = palette;
		pal =
		    (byte *) W_CacheLumpNum(lu_palette,
...
void ST_createWidgets(void)
{
	...
	STlib_initMultIcon(&w_keyboxes[0],
			   ST_KEY0X,
			   ST_KEY0Y, keys, &keyboxes[0], &st_statusbaron);

	STlib_initMultIcon(&w_keyboxes[1],
			   ST_KEY1X,
			   ST_KEY1Y, keys, &keyboxes[1], &st_statusbaron);

	STlib_initMultIcon(&w_keyboxes[2],
			   ST_KEY2X,
			   ST_KEY2Y, keys, &keyboxes[2], &st_statusbaron);
