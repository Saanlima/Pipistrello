...
unsigned long SwapBE32(unsigned long x)
{
    return ((x << 24) | ((x << 8) & 0x00FF0000) |
            ((x >> 8) & 0x0000FF00) | (x >> 24));
}

unsigned short SwapBE16(unsigned short x)
{
    return ((x << 8) | (x >> 8));
}
...
static boolean ReadByte(byte *result, FIL *stream)
{
    byte c;
    int count;

    f_read(stream, &c, 1, &count);
...
static boolean ReadVariableLength(unsigned int *result, FIL *stream)
{
    int i;
    byte b;
...
static void *ReadByteSequence(unsigned int num_bytes, FIL *stream)
{
    unsigned int i;
    byte *result;
...
static boolean ReadMetaEvent(midi_event_t *event, FIL *stream)
{
    byte b;
...
static boolean ReadTrackHeader(midi_track_t *track, FIL *stream)
{
    ...
    if (f_read(stream, &chunk_header, sizeof(chunk_header_t), &records_read))
      fprintf(stderr, "ReadTrackHeader: Can't read chuck header!\n");

    if (records_read < 1)
    {
        fprintf(stderr, "ReadTrackHeader: No chunk header read!\n");
        return false;
    }

    if (!CheckChunkHeader(&chunk_header, TRACK_CHUNK_ID))
    {
        fprintf(stderr, "ReadTrackHeader: Failed CheckChunkHeader!\n");
        return false;
    }

    track->data_len = SwapBE32(chunk_header.chunk_size);

    return true;
}

static boolean ReadTrack(midi_track_t *track, FIL *stream)
{
    ...
    {
        fprintf(stderr, "ReadTrack: Failed to track header\n");
        return false;
    }
    ...
    for (;;)
    {
        ...
        if (new_events == NULL)
        {
            fprintf(stderr, "ReadTrack: Failed to resize track\n");
            return false;
        }
        ...
        {
            fprintf(stderr, "ReadTrack: Failed to read event\n");
            return false;
        }
...
static boolean ReadAllTracks(midi_file_t *file, FIL *stream)
{
    unsigned int i;
    ...
    if (file->tracks == NULL)
    {
        fprintf(stderr, "ReadAllTracks: Failed to malloc tracks\n");
        return false;
    }
    ...
    for (i=0; i<file->num_tracks; ++i)
    {
        ...
        {
            fprintf(stderr, "ReadAllTracks: Failed to read track\n");
            return false;
        }
    }
...
static boolean ReadFileHeader(midi_file_t *file, FIL *stream)
{
    size_t records_read;
    unsigned int format_type;

    if (f_read(stream, &file->header, sizeof(midi_header_t), &records_read))
      fprintf(stderr, "ReadFileHeader: Can't read midi header!\n");
    ...
    if (!CheckChunkHeader(&file->header.chunk_header, HEADER_CHUNK_ID)
     || SwapBE32(file->header.chunk_header.chunk_size) != 6)
    {
        fprintf(stderr, "ReadFileHeader: Invalid MIDI chunk header! "
                        "chunk_size=%i\n",
                        SwapBE32(file->header.chunk_header.chunk_size));
        return false;
    }

    format_type = SwapBE16(file->header.format_type);
    file->num_tracks = SwapBE16(file->header.num_tracks);
...
midi_file_t *MIDI_LoadFile(char *filename)
{
    ...
    if (f_open(stream, filename, FA_OPEN_EXISTING | FA_READ))
    {
    ...
    {
        fprintf(stderr, "MIDI_LoadFile: Failed to read file header\n");
        f_close(stream);
        free(stream);
        MIDI_FreeFile(file);
        return NULL;
    ...
    {
        fprintf(stderr, "MIDI_LoadFile: Failed to read tracks\n");
        f_close(stream);
        free(stream);
        MIDI_FreeFile(file);
        return NULL;
